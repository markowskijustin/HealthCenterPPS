...
## Rate to Percent Served
```{r}
summary(lm(data = df.longer %>% mutate(value = ifelse(value > 300, 300, ifelse(value < 98, 98, value))) %>% filter(Year > 2018), Pct.MCD ~ value + Pct.NHB + Pct.HL + Pct.NHA +  Pct.Homeless + Pct.LessThan100pctFPL  + UrbanRuralFlag + factor(Year) + factor(HealthCenterState) ))

quantile(df.longer$value, probs = 0.025)
quantile(df.longer$value, probs = 0.975)

ggplot(data = df.longer %>% group_by(Year) %>% summarise(value = mean(Pct.MCD, na.rm = TRUE)), aes(x = Year, y = value)) + 
  geom_point()

ggplot(data = df.longer %>% group_by(Year) %>% summarise(value = mean(value, na.rm = TRUE)), aes(x = Year, y = value)) + 
  geom_point()
...
results.long <- oaxaca(data = df.longer %>% 
  ...
  summarise(AvgMCDrate = mean(value), 
