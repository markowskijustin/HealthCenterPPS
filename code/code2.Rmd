...
# Read in Data
```{r}
policyfeatures <- read_excel(path = "CHC_PPS_Crosswalk.xlsx", sheet = "PPSrateFeatures")
mcradjs <- read_excel(path = "CHC_PPS_Crosswalk.xlsx", sheet = "MedicareAdjustments") %>% rename(FIPS = `Included FIPS`, MCRadjustment = `2023 FQHC GAF`)
clinicrates <- read_excel(path = "CHC_PPS_Crosswalk.xlsx", sheet = "PPSRates")
```
...
model1 <- lm(data = df.modeling %>% filter(HealthCenterState %notin% c("CA","LA")), MCDrate ~ Pct.NHB + Pct.HL + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag + FundingCHC + FundingMHC + FundingHO + FundingPH)


model2 <- lm(data = df.modeling %>% filter(HealthCenterState %notin% c("CA","LA")), MCDrate ~ Pct.NHB + Pct.HL  +  Pct.Homeless + Pct.FarmWorker + Pct.NHA + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag + FundingCHC + FundingMHC + FundingHO + FundingPH)

model2.5 <- lm(data = df.modeling %>% filter(HealthCenterState %notin% c("CA","LA")), MCDrate ~ Pct.NHB * Pct.HL  +  Pct.Homeless + Pct.FarmWorker + Pct.NHA + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag + FundingCHC + FundingMHC + FundingHO + FundingPH)

model3 <- lm(data = df.modeling , MCDrate ~ Pct.NHB + Pct.HL + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag)

model4 <- lm(data = df.modeling , MCDrate ~ Pct.NHB + Pct.HL + Pct.NHA +  Pct.Homeless + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag )

model5 <- lm(data = df.modeling , MCDrate ~ Pct.NHB*Pct.HL + Pct.NHA +  Pct.Homeless + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag)
...
model <- lm(data = df.modeling , MCDrate ~ Pct.NHB + Pct.HL + Pct.Homeless + Pct.NHA + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag)
summary(model)


model <- lmer(data = df.modeling , MCDrate ~ Pct.NHB + Pct.HL + Pct.NHA + Pct.Homeless + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + (1|MCRadjustment)  + UrbanRuralFlag)
summary(model)$coeff
...
```{r}
model.need1 <- lm(data = df.fins , MCDrate ~ Pct.NHB+ Pct.HL  + Pct.NHA +  Pct.Homeless + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag)
model.need2 <- lm(data = df.fins , MCDrate ~ Pct.NHB + Pct.HL  + Need + Pct.Homeless + Pct.NHA +  Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag)
model.need3 <- lm(data = df.fins , MCDrate ~ Pct.NHB* Pct.HL + Need +  Pct.NHA +  Pct.Homeless + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag)
...
```{r}
summary(lm(data = df.fins, Need ~ Pct.NHB + Pct.HL))
```
...
results <- oaxaca(data = df.fins  %>% 
  ...
  MCDrate ~ AvgMCDrate + CostPerPatient + Need  + Pct.Homeless + MCRadjustment  | Split, R = 1000, cluster = df.fins$BHCMISID)
...
$$ R = ( \bar{X}_1 - \bar{X}_2 ) \beta_2 + \bar{X}_2 ( \beta_1 - \beta_2 ) + ( \bar{X}_1 - \bar{X}_2 ) ( \beta_1 - \beta_2 ) $$
...
mean(df.kob[df.kob$Split == 1,]$Total * df.kob[df.kob$Split == 1,]$Pct.MCD / 100) * 2.5 * (results$threefold$overall[5])
mean(df.kob[df.kob$Split == 1,]$Total) * 2.5 * (results$threefold$overall[3] + results$threefold$overall[5])


nrow(df.kob[df.kob$ngla_net_gain_loss < 0 & !is.na(df.kob$ngla_net_gain_loss),])/nrow(df.kob[!is.na(df.kob$ngla_net_gain_loss),])

nrow(df.kob[df.kob$ngla_net_gain_loss>mean(df.kob[df.kob$Split == 1,]$Total) * 2.5 * (results$threefold$overall[5]) & df.kob$ngla_net_gain_loss < 0 & !is.na(df.kob$ngla_net_gain_loss),])/nrow(df.kob[!is.na(df.kob$ngla_net_gain_loss),])
...
results$threefold$overall
results$threefold$overall[c(1,3,5)] / results$threefold$overall[c(2,4,6)]
(round(results$threefold$overall[c(1,3,5)] / results$y$y.diff * 100))
...
rbind(results$threefold$overall, results$threefold$variables) %>% 
  ...
  pack_rows(index = c("Overall Impacts" = 1, "Covariates" = 6)) %>% 
