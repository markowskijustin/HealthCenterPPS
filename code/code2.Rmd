...
splits <- median(df.fins[is.na(df.fins$Tribal) & df.fins$HealthCenterState %notin% c("OR"),]$Pct.NHB + df.fins[is.na(df.fins$Tribal)& df.fins$HealthCenterState %notin% c("OR"),]$Pct.HL)
cuts_Cost <- quantile(df.fins[is.na(df.fins$Tribal) & df.fins$HealthCenterState %notin% c("OR"),]$CostPerPatient, probs = c(0.025, 0.975), na.rm = TRUE)
cuts_Rate <- quantile(df.fins[is.na(df.fins$Tribal) & df.fins$HealthCenterState %notin% c("OR"),]$MCDrate, probs = c(0.025, 0.975), na.rm = TRUE)
...
results <- oaxaca(data = df.fins  %>% 
                    ...
                    #filter(is.na(Tribal)) %>% 
                    filter(is.na(Tribal) & HealthCenterState %notin% c("OR")) %>% 
...
rbind(results$threefold$overall, results$threefold$variables) %>% 
 ...
 df.fins  %>% filter(!is.na(fs_miscellaneous))
