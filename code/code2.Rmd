...
## Comparing Dental
```{r}
clinicrates <- read_excel(path = "/Users/jhm65/Dropbox/Git/HealthCenterPPS/data/CHC_PPS_Crosswalk.xlsx", sheet = "PPSRates")


ratesbyservice <- clinicrates %>% 
 filter(!is.na(Dental) & HealthCenterState %notin% c("WI")) %>% 
  mutate(Medical = ifelse(is.na(Medical), Values, Medical)) %>% 
  select(HealthCenterName...1, HealthCenterState, Medical, Dental, BH) %>% 
  pivot_longer(cols = c(Medical, Dental, BH)) 

## Plain boxplot by service line
p <- ggplot(data = ratesbyservice, aes(x = value, y = name)) + geom_boxplot() + xlab("PPS Rates ($)") + ylab("Service Line")
suppressWarnings(print(p))

## Regression of differences
summary(lm(data = ratesbyservice, value ~ name))
rqfit <- rq(data = ratesbyservice, value ~ name)
summary(rqfit)

## Facet Wrap to show differences by service line across states
p <- ggplot(data = ratesbyservice, aes(x = value, y = name)) + geom_boxplot() + facet_wrap(~HealthCenterState) + xlab("PPS Rate ($)") + ylab("Service Line")
suppressWarnings(print(p))
p <- ggplot(data = ratesbyservice, aes(x = value, y = name)) + geom_boxplot() + facet_wrap(~HealthCenterState, scales = "free_x") + xlab("PPS Rate ($)") + ylab("Service Line")
suppressWarnings(print(p))


## Comparisons of Means
p <- ratesbyservice %>% 
  group_by(HealthCenterState, name) %>% 
  summarise(PPS = mean(value, na.rm = TRUE)) %>% 
  rename(ServiceLine = name) %>% 
  ggplot(aes(x = HealthCenterState, y = PPS, fill = ServiceLine)) + geom_bar(stat='identity', position='dodge') + theme_tufte_revised() + 
  ggtitle("Average PPS Rates by Service Line and State")
suppressWarnings(print(p))
...
#df.geo <- df.geo %>% filter(HealthCenterState != "OR")
...
mechiv <- ivreg(MCDrate ~  Ratio +  Need + CostPerPatient | Pct.NHB + Pct.HL + Pct.NHA +  Pct.Homeless + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + HealthCenterState + I(MCRadjustment*100) + UrbanRuralFlag, data = df.fins)
summary(mechiv)




stargazer(mechols.1, mechols.2, mechols.3, mechols.4, mechiv, type = 'text', omit = c("HealthCenterState","Constant"))
...
splits <- median(df.fins[is.na(df.fins$Tribal) & df.fins$HealthCenterState %notin% c(),]$Pct.NHB + df.fins[is.na(df.fins$Tribal)& df.fins$HealthCenterState %notin% c(),]$Pct.HL)


cuts_Cost <- quantile(df.fins[is.na(df.fins$Tribal) & df.fins$HealthCenterState %notin% c(),]$CostPerPatient, probs = c(0.025, 0.975), na.rm = TRUE)
cuts_Rate <- quantile(df.fins[is.na(df.fins$Tribal) & df.fins$HealthCenterState %notin% c(),]$MCDrate, probs = c(0.025, 0.975), na.rm = TRUE)
...
results <- oaxaca(data = df.fins  %>% 
                    ...
                    #filter(is.na(Tribal)) %>% 
                    filter(is.na(Tribal) & HealthCenterState %notin% c()) %>% 
  ...
  MCDrate ~ AvgMCDrate + CostPerPatient + Need + MCRadjustment + Pct.MCD | Split, R = 1000)
  #MCDrate ~ AvgMCDrate + CostPerPatient + Need + Pct.NHA + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None + I(MCRadjustment*100) + UrbanRuralFlag| Split, R = 1000, cluster = df.fins$BHCMISID)
